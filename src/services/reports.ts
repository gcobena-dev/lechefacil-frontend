...
export interface ProductionReportData {
  ...
  daily_by_animal?: Record<
    string,
    Record<string, { weight_lb: number; total_liters: number }>
  >;
  animals?: Array<{
    id: string;
    tag: string;
...
export async function generateAnimalsReport(
  params: ReportRequest
): Promise<ReportResponse> {
  ...
  if (Capacitor.isNativePlatform()) {
    (async () => {
      try {
        ...
        const base64 = report.content.includes(",")
          ? report.content.split(",")[1]
          : report.content;
