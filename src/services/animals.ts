...
export async function updateAnimal(
  id: string,
  payload: Partial<
    Omit<AnimalResponse, "id" | "tenant_id" | "created_at" | "updated_at">
  > & { version: number }
) {
  return apiFetch<AnimalResponse>(`/api/v1/animals/${id}`, {
    method: "PUT",
...
export async function getPhotoUploadUrl(animalId: string, contentType: string) {
  return apiFetch<AnimalPhotoUploadResponse>(
    `/api/v1/animals/${animalId}/photos/uploads`,
    {
      method: "POST",
      withAuth: true,
      withTenant: true,
...
export async function updateAnimalPhoto(
  ...
  return apiFetch<AnimalPhotoResponse>(
...
export async function uploadAnimalPhoto(
  ...
  const { upload_url, storage_key, fields } = await getPhotoUploadUrl(
    animalId,
    file.type
  );
