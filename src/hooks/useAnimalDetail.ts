...
export function useAnimalDetail(animalId: string) {
  ...
  const productionQuery = useQuery({
    queryKey: ["animal-production", animalId, formatDate(ninetyDaysAgo), formatDate(today)],
  ...
  const processedData: AnimalDetailData | null = animalQuery.data && productionQuery.data ? {
    ...
    productionSummary: (() => {
      ...
      const totalLiters = productions.reduce((sum, p) => sum + parseFloat(p.volume_l), 0);
      const totalEarnings = productions.reduce((sum, p) => {
        const price = p.price_snapshot ? parseFloat(p.price_snapshot) : defaultPrice;
      ...
      const thirtyDaysProductions = productions.filter(p =>
        new Date(p.date_time) >= thirtyDaysAgo
      );
      const last30DaysLiters = thirtyDaysProductions.reduce((sum, p) => sum + parseFloat(p.volume_l), 0);

      // Calculate unique days with records in the last 30 days
      const uniqueDaysLast30 = new Set(
        thirtyDaysProductions.map(p => new Date(p.date_time).toDateString())
      ).size;
      const last30DaysAvg = uniqueDaysLast30 > 0 ? last30DaysLiters / uniqueDaysLast30 : 0;

      // Calculate earnings for last 30 days
      const last30DaysEarnings = thirtyDaysProductions.reduce((sum, p) => {
        const price = p.price_snapshot ? parseFloat(p.price_snapshot) : defaultPrice;
        return sum + (parseFloat(p.volume_l) * price);
      }, 0);
