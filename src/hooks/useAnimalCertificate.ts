...
export const useCreateCertificate = (animalId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['animal-certificate', animalId] });
      toast.success(t('animals.certificateCreated'));
    },
    onError: (error: any) => {
      const message = error.response?.data?.detail || t('animals.certificateCreateError');
...
export const useUpdateCertificate = (animalId: string) => {
  ...
  return useMutation<AnimalCertificate, Error, CertificateUpdatePayload>({
    ...
    onError: (error: any) => {
      const message = error.response?.data?.detail || t('animals.certificateUpdateError');
      toast.error(String(message));
    },
  });
};
...
export const useDeleteCertificate = (animalId: string) => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      const message = error.response?.data?.detail || t('animals.certificateDeleteError');
      toast.error(String(message));
    },
  });
};
